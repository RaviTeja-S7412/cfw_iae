        	...
        	<div class="col-md-12">
				<div class="card card-primary card-outline">
					<div class="card-header">
						<h3 class="card-title"><? echo ($id != "") ? 'Update' : 'Create'; ?> Semester</h3>
					</div>
					<div class="card-body">
        		...
        		<div class="card card-primary card-outline">
				  ...
				  <div class="card-body">
					<table id="example1" class="table table-bordered table-hover">
					  ...
					  <tbody>
						  <? $data = $this->db->order_by("id","desc")->get_where("tbl_semesters",["deleted"=>0])->result(); 
						  		$i = 1;
								foreach($data as $d){
						  ?>
							  <tr>
...
<script>
		...
		.then((willDelete) => {
		  if (willDelete) {
			  	...
			  	$.ajax({
					...
					data : {id:id},
					dataType: "json",
					url : "<? echo base_url('apis/Semester/delete') ?>",
	...
	$("#cprogram").submit(function(e){
		...
		$.ajax({
			...
			data : fdata,
			dataType: "json",
			url : "<? echo base_url('apis/Semester/create') ?>",
			success : function(data){
